...
/** Inline upload flow for the "Update text" screen — extracted from the old
 * standalone /upload route. Reports the extracted text via onExtracted
 * instead of routing away, so the caller can drop straight into its own
 * editor state. */
export function UploadPanel({ onExtracted }) {
  ...
  function openInEditor() {
    ...
    onExtracted({ title, text: result?.text || "" });
    setStage(0);
    setResult(null);
  }
