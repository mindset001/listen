 ...
 *
 * .env (server):
 *   TTS_API_KEY=UAPI-...        // rotate the key that was shared in chat
...
export async function generateSpeech(text, voice, options = {}) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403) throw new TTSError("auth", "Speech service rejected the credentials", res.status);
    if (res.status === 404) throw new TTSError("voice", "That voice is not available", res.status);
    if (res.status === 413) throw new TTSError("too_long", "That text is too long for one request", res.status);
    if (res.status === 429) throw new TTSError("rate_limit", "Too many requests — try again shortly", res.status);
