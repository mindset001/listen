...
struct SavedAudioView: View {
    ...
    private var savedAudio: [Document] { library.documents.filter { $0.audio } }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Theme.Space.base) {
                ...
                VStack(spacing: 10) {
                    ForEach(savedAudio) { doc in
                        let voiceName = library.voices.first { $0.id == doc.voice }?.name ?? (doc.voice ?? "")
                        HStack(spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                ...
                                Text("\(doc.duration) · \(voiceName) · \(doc.date)")
